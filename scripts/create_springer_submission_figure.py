...
OUTPUT = ROOT / "outputs" / "paper_assets" / "figure_1_springer_submission_600dpi.png"
...
F_PANEL = load_font(ARIAL_BOLD, 76)
F_TITLE = load_font(ARIAL_BOLD, 64)
F_BODY = load_font(ARIAL, 64)
F_MATH = load_font(TIMES_ITALIC, 66)
F_MATH_SCRIPT = load_font(TIMES_ITALIC, 46)


def text_width(draw: ImageDraw.ImageDraw, text: str, used_font: ImageFont.FreeTypeFont) -> float:
    box = draw.textbbox((0, 0), text, font=used_font)
    return box[2] - box[0]


def wrap_line(draw: ImageDraw.ImageDraw, text: str, used_font: ImageFont.FreeTypeFont,
...
def wrap_text(draw: ImageDraw.ImageDraw, text: str, used_font: ImageFont.FreeTypeFont,
              max_width: int) -> str:
    lines: list[str] = []
    for source_line in text.split("\n"):
        lines.extend(wrap_line(draw, source_line, used_font, max_width))
    return "\n".join(lines)


def fit_wrapped_text(draw: ImageDraw.ImageDraw, text: str,
                     used_font: ImageFont.FreeTypeFont, max_width: int,
                     max_height: int, spacing: int,
                     min_size: int = 50) -> tuple[ImageFont.FreeTypeFont, str]:
...
def centered(draw: ImageDraw.ImageDraw, xy: tuple[int, int, int, int], text: str,
             used_font: ImageFont.FreeTypeFont, fill: str = INK, spacing: int = 16) -> None:
    x1, y1, x2, y2 = xy
    used_font, wrapped = fit_wrapped_text(
        draw, text, used_font, x2 - x1, y2 - y1, spacing
    )
    box = draw.multiline_textbbox((0, 0), wrapped, font=used_font, spacing=spacing, align="center")
    width = box[2] - box[0]
    height = box[3] - box[1]
    draw.multiline_text(
...
def _math_spans(text: str) -> list[tuple[str, str]]:
...
def centered_math(draw: ImageDraw.ImageDraw, xy: tuple[int, int, int, int], text: str,
                  fill: str) -> None:
    x1, y1, x2, y2 = xy
    lines = text.split("\n")
    line_height = 92
    ...
    for line_number, line in enumerate(lines):
        spans = _math_spans(line)
        widths = [text_width(draw, value, F_MATH if mode == "base" else F_MATH_SCRIPT)
        ...
        for (value, mode), width in zip(spans, widths):
            used_font = F_MATH if mode == "base" else F_MATH_SCRIPT
            y = baseline_y
            if mode == "sub":
                y += 34
            elif mode == "sup":
                y -= 18
            draw.text((cursor, y), value, font=used_font, fill=fill)
            cursor += width


def box(draw: ImageDraw.ImageDraw, xy: tuple[int, int, int, int], title: str,
        body: str, *, fill: str, outline: str, math: str | None = None,
        title_height: int = 120, math_height: int = 150) -> None:
    x1, y1, x2, y2 = xy
...
def create_figure(path: Path = OUTPUT) -> Path:
    ...
    # Panel (a): model information flow.
    ...
    box(
        ...
        (120, 240, 590, 1740),
        "Inputs",
        "Two directed knowledge graphs\n\nRelation triples (h, r, t)\n\nNames, relation terms, and attributes\n\nSeed entity pairs",
        fill="#F6F7F8",
        outline=LINE,
    )
    ...
    box(
        draw,
        (690, 240, 1810, 900),
        "Relation-aware structural branch",
        "Topology statistics + shared projection\nRelation-aware incoming messages\nNode-level layer selection",
        fill=STRUCT_FILL,
        outline=STRUCT,
    ...
    box(
        draw,
        (690, 1020, 1810, 1740),
        "Multi-scale semantic branch",
        "One shared token sequence\nToken, phrase, and global views\nEntity-level view gate",
        fill=SEM_FILL,
        outline=SEM,
    ...
    box(
        draw,
        (1930, 240, 3120, 1740),
        "Semantics-guided context",
        "Complete one-hop outgoing neighborhood\nSemantic query + validity mask\n1.5-entmax sparse weights\nSelf/neighbor context gate\nFinal structural-semantic gate",
        fill=FUSION_FILL,
        outline=FUSION,
    ...
    # Panel (b): the shared training and retrieval representation.
    ...
    box(
        ...
        (1425, 2120, 2385, 2595),
        "Primary retrieval",
        "Training and retrieval use\nthe same joint representation",
        fill=OUTPUT_FILL,
        outline=OUTPUT_LINE,
    ...
    box(
        ...
        (2600, 2120, 3560, 2595),
        "Validation control",
        "Validation selects only\nthe CSLS neighborhood size",
        fill="#F6F7F8",
        outline=LINE,
